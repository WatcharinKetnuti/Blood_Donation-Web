        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <?php
                    ...
                    // เริ่มแสดงตาราง
                    echo '<table id="datatablesSimple" class="mb-3">';
                    echo '<thead>
                            <tr>
                                <th>รหัสการจอง</th>
                                <th>วันที่จอง</th>
                    ...
                    if($result !== false) {
                        foreach($result as $row) {
                            ...
                            if ($status === 'D') $count_donated++;
                            else if ($status === 'W') $count_waiting++;
                            else if ($status === 'C') $count_cancel++;

                            //$statusText = $status === 'W' ? 'ยังไม่บริจาค' : ($status === 'D' ? 'บริจาคแล้ว' : 'ยกเลิก');
                            //$statusClass = $status === 'D' ? 'text-success' : ($status === 'C' ? 'text-danger' : 'text-secondary');
                    ...
                    echo "<p class='fw-bold'>
                        <span class='text-success'>บริจาคแล้ว: {$count_donated} คน</span>,
                        <span class='text-secondary'>ยังไม่บริจาค: {$count_waiting} คน</span>,
                        <span class='text-danger'>ยกเลิก: {$count_cancel} คน</span>
                    </p>";
                ?>
            </div>
        </div>
